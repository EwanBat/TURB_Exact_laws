...
class SourceRdugyrdv(SourceRduisodv):
    ...
    def calc(self, vector: List[int], cube_size: List[int], rho, ugyr, divv, traj=False, **kwarg) -> List[float]:
        if traj:
            return calc_source_with_numba_traj(calc_in_point_with_sympy_traj, *vector, *cube_size, rho, ugyr, divv)
