...
class SourceRvdpisodr(AbstractTerm):
    ...
    def set_sympy_expr(self):
        ...
        self.expr = rhoNP * dpiso * scalprod / rhoP 
...
def print_expr():
    sp.init_printing(use_latex=True)
    return SourceRvdpisodr().expr

@njit
def calc_in_point_with_sympy(i, j, k, ip, jp, kp, rho, vx, vy, vz, piso, dxrho, dyrho, dzrho, f=njit(SourceRvdpisodr().fct)):
...
@njit
def calc_in_point_with_sympy_traj(t, tp, rho, vx, vy, vz, piso, dxrho, dyrho, dzrho, f=njit(SourceRvdpisodr().fct)):
