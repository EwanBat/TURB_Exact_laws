...
class AbstractTerm:
    ...
    def _calc_incremental_trajectories_loop(self, merged_quantities, n_trajectories, n_points, max_workers=2):
        ...
        def process_single_trajectory(i): # Process a single trajectory and return its results
            ...
            for dl in range(n_points):
                trajectory_results[dl] = self.calc([dl+n_points], [n_points], **merged_quantities[i], traj=True)
    ...
    def calc_incremental_trajectories(self, dic_quantities: dict, traj_param: dict, sat1:str, sat2:str, max_workers: int = None) -> (float or List[float]):
        n_trajectories = traj_param["n_trajectories"]
        n_points = traj_param["n_points"]
        # Priority: explicit parameter > environment variable > default (2)
        if max_workers is None:
            max_workers = int(os.environ.get('THREADPOOL_MAX_WORKERS', 2))
        ...
        return self._calc_incremental_trajectories_loop(merged_quantities, n_trajectories, n_points, max_workers)        
