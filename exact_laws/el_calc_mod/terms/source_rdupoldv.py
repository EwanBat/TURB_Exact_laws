...
class SourceRdupoldv(SourceRduisodv):
    ...
    def calc(self, vector: List[int], cube_size: List[int], rho, upol, divv, traj=False, **kwarg) -> List[float]:
        if traj:
            return calc_source_with_numba_traj(calc_in_point_with_sympy_traj, *vector, *cube_size, rho, upol, divv)
        return calc_source_with_numba(calc_in_point_with_sympy, *vector, *cube_size, rho, upol, divv)

    def calc_fourier(self, rho, upol, divv, traj=False, **kwarg) -> List:
        return calc_with_fourier(rho, upol, divv, traj=traj)
