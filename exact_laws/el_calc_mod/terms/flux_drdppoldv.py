...
class FluxDrdppoldv(FluxDrdpisodv):
    ...
    def calc(self, vector:List[int], cube_size:List[int], rho, ppol, vx, vy, vz, traj=False, **kwarg) -> List[float]:
        if traj:
            return calc_flux_with_numba_traj(calc_in_point_with_sympy_traj, *vector, *cube_size, rho, ppol, vx, vy, vz)
        return calc_flux_with_numba(calc_in_point_with_sympy, *vector, *cube_size, rho, ppol, vx, vy, vz)

    def calc_fourier(self, rho, ppol, vx, vy, vz, traj=False, **kwarg) -> List:
        return calc_with_fourier(rho, ppol, vx, vy, vz, traj=traj)
