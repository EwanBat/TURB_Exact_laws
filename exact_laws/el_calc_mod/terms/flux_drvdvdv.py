...
def calc_with_fourier(rho, vx, vy, vz, traj=False):
    ...
    frvxvx = transform(rho*vx*vx)
    frvyvy = transform(rho*vy*vy)
    frvzvz = transform(rho*vz*vz)
    
    fvxvx = transform(vx*vx)
    fvyvx = transform(vy*vx)
    fvzvx = transform(vz*vx)
    frvyvx = transform(rho*vy*vx)
    frvzvx = transform(rho*vz*vx)
    flux_x = inv_transform(fvx*np.conj(frvxvx+frvyvy+frvzvz) - np.conj(fvx)*(frvxvx+frvyvy+frvzvz) 
    ...
    fvzvy = transform(vz*vy)
    frvzvy = transform(rho*vz*vy)
    flux_y = inv_transform(fvy*np.conj(frvxvx+frvyvy+frvzvz) - np.conj(fvy)*(frvxvx+frvyvy+frvzvz) 
    ...
    flux_z = inv_transform(fvz*np.conj(frvxvx+frvyvy+frvzvz) - np.conj(fvz)*(frvxvx+frvyvy+frvzvz) 
